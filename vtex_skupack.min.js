...
var apply_sku_selection=function(){$.browser.msie&&$("*[name*='espec_']").click(function(){$(this).is("input:radio")&&(this.blur(),this.focus())});$("*[name*='espec_']").change(function(){var a=[],c=[],d="",e=0,a=getSelected();if(void 0===a[0].name)enableAllOptions();else{if(1<myJSONSkuSpecification.specifications){var e=0,f=Array(a.length);$.each(a,function(a,b){d=b.name;especPos=d.replace("espec_","");var c="";$.each(myJSONSkuSpecification.combination,function(a,d){if(d[b.value]){var g=0;f[e]=Array(d[b.value].length+
1);$.each(d[b.value],function(a,b){especPos==g&&g++;c="espec_"+g;"undefined"!=b[c]&&(f[e][g]=b[c]);g++})}});f[e][especPos]=getValuesOptions(d);e++});$.each(f,function(a,b){0==a?$.each(b,function(a,b){c[a]=b.split(",")}):$.each(b,function(a,b){c[a]=b.split(",").intersect(c[a])})});for(j=0;j<myJSONSkuSpecification.specifications;j++)a="*[name='espec_"+j+"']",$(a).is("input:radio")?$(a).each(function(){$(this).attr("disabled","disabled");$("label[for='"+$(this).attr("id")+"']").removeClass("sku-picked")}):
($(a).find("option").each(function(){$(this).attr("disabled","disabled")}),$(a).find("option:first").each(function(){$(this).removeAttr("disabled")}));enableOptions(c);checkSelected(c)}else for(j=0;j<myJSONSkuSpecification.specifications;j++)a="*[name='espec_"+j+"']",$(a).is("input:radio")&&$(a).each(function(){$("label[for='"+$(this).attr("id")+"']").removeClass("sku-picked")});$("label[for='"+$(this).attr("id")+"']").addClass("sku-picked");getSku();notifymeOnButtomOkClick()}});ImageControl($("ul.thumbs a:first"),
0);clickThumbs();var a=new Vtex.JSEvents.Listener("imageControlListener",imageControl_OnSkuDataReceived);skuEventDispatcher.addListener(skuDataReceivedEventName,a);a=new Vtex.JSEvents.Listener("imageControlSpecSelectedListener",imageControl_OnSkuImageRelatedSpecSelected);skuEventDispatcher.addListener(skuImageRelatedSpecSelectedEventName,a)};
...
function enableOptions(a){$.each(a,function(a,c){$.each(c,function(d,c){var f="[name='espec_"+a+"']",f=$(f).is("input:radio")?"input"+f+"[value='"+c+"']":"select"+f+" option[value="+c+"]";$(f).removeAttr("disabled")})})}function enableAllOptions(){for(j=0;j<myJSONSkuSpecification.specifications;j++){var a="*[name='espec_"+j+"']";$(a).is("input:radio")?$(a).each(function(){$(this).removeAttr("disabled")}):$(a).find("option").each(function(){$(this).removeAttr("disabled")})}}
...
function imageControl_OnSkuImageRelatedSpecSelected(a){a=getSkuData(a.newSkuId);var b=new Vtex.Commerce.JSEvents.SkuDataReceivedEventArgs;b.skuData=a;imageControl_OnSkuDataReceived(b)}
function imageControl_OnSkuDataReceived(a){$("ul.thumbs").html("");var b=[],c=0,d=0,e=0,f="",g="",k="",h=a.productIndex;void 0===h&&(h=0);for(i=0;i<a.skuData.images.length;i++){for(j=e=d=0;j<a.skuData.images[i].length;j++)3==a.skuData.images[i][j].ArchiveTypeId&&(e=a.skuData.images[i][j].IdArchive,k=a.skuData.images[i][j].Path),2==a.skuData.images[i][j].ArchiveTypeId&&(c=a.skuData.images[i][j].IdArchive,f=a.skuData.images[i][j].Path),10==a.skuData.images[i][j].ArchiveTypeId&&(d=a.skuData.images[i][j].IdArchive,
g=a.skuData.images[i][j].Path);0<c&&0<e&&(b[i]=$("<li></li>"),e=$("<a></a>").attr("rel",f).attr("title","Zoom").attr("href","javascript:void(0);").attr("id","botaoZoom").attr("class",""),0<d?e.attr("zoom",g):e.attr("zoom",""),d=$("<img />").attr("title",a.skuData.name).attr("src",k),e.append(d),b[i].append(e))}for(i=0;i<b.length;i++)$("ul.thumbs").append(b[i]);ImageControl($("ul.thumbs a:first"),h);clickThumbs()}function clickThumbs(){$("ul.thumbs a").click(function(){ImageControl(this,0)})}
function ImageControl(a,b){$("ul.thumbs a").removeClass("ON");$(a).attr("id");var c=$(a).offset(),d=$("#produto").offset(),e=$("[id=show] [id=include] [id=image][productIndex="+b+"] img").attr("alt"),f=$("<img />");$(a).addClass("ON");$("div#setaThumbs").css({left:c.left-d.left+30+"px"});0<$("[id=show] [id=include] [id=image][productIndex="+b+"] a").length?$("[id=show] [id=include] [id=image][productIndex="+b+"] a").remove():$("[id=show] [id=include] [id=image][productIndex="+b+"] img").remove();
0==b?f.attr("id","image-main").attr("src",$(a).attr("rel")).attr("productIndex",b).attr("class","sku-rich-image-main"):f.attr("id","image-main").attr("src",$(a).attr("rel")).attr("productIndex",b);""==$(a).attr("zoom")?(f.attr("alt",e).attr("title",e),$("[id=show] [id=include] [id=image][productIndex="+b+"]").append(f)):(c=$("<a></a>").addClass("image-zoom").attr("href",$(a).attr("zoom")),c.append(f),$("[id=show] [id=include] [id=image][productIndex="+b+"]").append(c),LoadZoom(b))}
function LoadZoom(a){if(0<$(".image-zoom").length){var b=300,c=300,d=$("<div></div>");d.addClass("jqZoomWindow");$("[id=show] [id=include] [id=image][productIndex="+a+"]").append(d);"auto"!=d.css("width")&&null!=d.css("width")&&(b=parseInt(d.css("width").replace("px","")));"auto"!=d.css("height")&&null!=d.css("height")&&(c=parseInt(d.css("height").replace("px","")));d.remove();a={preloadText:"",zoomWidth:b,zoomHeight:c};$(".image-zoom").jqzoom(a)}};